...
@search_wrap(template="bmf-search_results.html",
             ...
             cleaners={"level_number": lambda v: v['level_label'].split(".")[1],
                       ...
                       "sfe": lambda v: "+1" if v.get('sfe',
                                                      None) == 1 else ("-1" if v.get('sfe',
                                                                                     None) == -1 else "?"),
                       "url": lambda v: url_for('.render_bmf_webpage',
                       ...
                       "cm": lambda v: cm_info(v.pop('CM',
                                                     '?')),
                       "field_knowl": lambda e: nf_display_knowl(e['field_label'],
...
def bmf_field_dim_table(**args):
    ...
    if level_flag == 'all':
        ...
        # Only get records where the cuspdial/new dimension is positive for
        # some weight
    ...
    dimtable = [{'level_label': dat['level_label'],
                 ...
                 'level_space': url_for(".render_bmf_space_webpage",
                                        field_label=field_label,
                                        level_label=dat['level_label']) if gl_or_sl == 'gl2_dims' else "",
                 'dims': dims[dat['level_label']]} for dat in data]
    info['dimtable'] = dimtable
    return render_template(
        "bmf-field_dim_table.html",
        info=info,
        title=t,
...
def render_bmf_space_webpage(field_label, level_label):
    ...
    bread = get_bread([(field_pretty(field_label), url_for(
        ".render_bmf_field_dim_table_gl2", field_label=field_label)), (level_label, '')])
    friends = []
    properties = []
        ...
        if not db.bmf_dims.exists({'field_label': field_label}):
            ...
            if not data:
                ...
                newforms = db.bmf_forms.search(
                    {'field_label': field_label, 'level_label': level_label})
                info['nfdata'] = [{'label': f['short_label'],
                                   'url': url_for(".render_bmf_webpage",
                ...
                # currently we have newforms of dimension 1 and 2 only (mostly
                # dimension 1)
...
def how_computed_page():
    ...
    return render_template(
        "double.html",
        kid='rcs.source.mf.bianchi',
        kid2='rcs.ack.mf.bianchi',
        title=t,
        bread=bread,
...
class BianchiStats(StatsDisplay):
    ...
    def short_summary(self):
        return r'The database currently contains %s %s of weight 2 over the nine imaginary quadratic fields of class number one.  Here are some <a href="%s">further statistics</a>.' % (
            comma(self.nforms), display_knowl("mf.bianchi.bianchimodularforms", "Bianchi modular forms"), url_for(".statistics"))
