...
def knowl_cache(galois_labels):
    ...
    gap_labels = []
    for rec in db.gps_transitive.search({"label": {"$in": galois_labels}}, [
...
def smallgroup_cache(gap_labels, cache=None, reverse=None):
    ...
    for rec in db.gps_small.search(
            {"label": {"$in": gap_labels}}, ["label", "pretty"]):
        label = rec["label"]
        cache[label] = rec
...
def small_group_display_knowl(n, k, name=None, cache=None):
    ...
    return '<a title = "' + myname + \
        ' [group.small.data]" knowl="group.small.data" kwargs="gapid=' + str(n) + '.' + str(k) + '">' + myname + '</a>'
...
def small_group_label_display_knowl(label, name=None):
    if not name:
        ...
        name = '$%s$' % group['pretty']
    return '<a title = "' + name + \
        ' [group.small.data]" knowl="group.small.data" kwargs="gapid=' + label + '">' + name + '</a>'
...
def small_group_data(gapid):
    ...
    inf += '<br>Normal subgroups: ' + \
        display_multiset(group['normal_subgroups'], small_group_label_display_knowl)
    inf += '<br>Maximal subgroups: ' + \
        display_multiset(group['maximal_subgroups'], small_group_label_display_knowl)
    inf += '<br>Center: ' + \
        small_group_label_display_knowl(str(group['center']))
    inf += '<br>Derived subgroup: ' + \
        small_group_label_display_knowl(str(group['derived_group']))
    inf += '<br>Abelianization: ' + \
        small_group_label_display_knowl(str(group['abelian_quotient']))
...
def list_with_mult(lis, names=True):
    ...
    for k in lis:
        ...
        if names:
            ...
            ans += group_display_knowl(k[0][0], k[0]
                                       [1], base_label(k[0][0], k[0][1]))
        if k[1] > 1:
    ...
    if pretty != '':
        ...
        if useknowls:
            ...
            if gapgroup is not None:
                pretty = small_group_display_knowl(
                    group['order'],
                    group['gapid'],
                    name='$' + gapgroup['pretty'] + '$',
                    cache=cache)
        if skip_nTj:
...
def group_display_knowl(n, t, name=None):
    ...
    if group is None:
        return name
    return '<a title = "' + name + \
        ' [nf.galois_group.data]" knowl="nf.galois_group.data" kwargs="n=' + str(n) + '&t=' + str(t) + '">' + name + '</a>'
...
def group_display_inertia(code):
    ...
    ans = "Intransitive group isomorphic to " + \
        small_group_display_knowl(code[1][0], code[1][1])
    return ans
