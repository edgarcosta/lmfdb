...
class mf_gamma1(MfChecker):
    ...
    def check_box_count(self):
        ...
        return accumulate_failures(
            self.check_count(
                box['Nk_count'],
                make_query(box)) for box in db.mf_boxes.search())
        ...
        return accumulate_failures(
            self.check_non_null(
                ...
                self._box_query(
                    ...
                    drop=[
                        'char_order',
                        'dim'])) for box in db.mf_boxes.search(
                {
                    'omin': None,
                    'omax': None,
                    'Dmin': None,
    ...
    def check_newspaces_hecke_orbit_dims(self):
        ...
        return self.check_crosstable_aggregate(
            'mf_newforms', 'hecke_orbit_dims', [
                'level', 'weight'], 'dim', sort=[
                'char_orbit_index', 'hecke_orbit'])
    ...
    def check_newspaces_num_spaces(self):
        ...
        # self.check_crosstable_count('char_dir_orbits', 'num_spaces',
        # ['level', 'weight_parity'], ['modulus', 'parity']))
    ...
    def check_oldspace_decomposition_totaldim(self):
        ...
        return self.check_crosstable_dotprod(
            'mf_gamma1_subspaces', 'cusp_dim', 'label', [
                'sub_mult', 'sub_dim'])
    ...
    def check_portraits_count(self):
        ...
        return self.check_crosstable_count(
            'mf_gamma1_portraits', 1, 'label', constraint={
                'dim': {
                    '$gt': 0}, 'level': {
                    '$lte': 4000}})

    ### slow ###
    @slow(projection=['level',
                      'level_radical',
                      'level_primes',
                      'level_is_prime',
