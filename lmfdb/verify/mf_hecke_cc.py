...
class mf_hecke_cc(MfChecker):
    ...
    def check_conrey_indexes(self):
        ...
        """
        # ps: In check_embedding_m and check_embedding_index, we already
        # checked that embedding_m and  check_embedding_index are in an
        # increasing sequence
    ...
    def check_an_length(self):
        ...
        return self._run_query(
            SQL("array_length({0}, 1) < 1000 OR array_length({0}, 2) != 2").format(
    ...
    def check_label_conrey(self):
        ...
        # TIME about 230s
        return self._run_query(
            SQL("(string_to_array({0},'.'))[5:6] != array[{1}::text,{2}::text]").format(
                Identifier('label'),
                Identifier('conrey_index'),
