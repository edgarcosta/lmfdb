...
class char_dir_orbits(TableChecker):
    ...
    def check_char_dir_values_agg(self):
        ...
        # TIME about 750s
        return self.check_crosstable_count(
            'char_dir_values', 'char_degree', 'orbit_label')
    ...
    def check_is_primitive(self):
        ...
        """
        # TIME about 1s
        # Since we can't use constraint on modulus=conductor, we construct the
        # constraint directly
    ...
    def check_galois_orbit(self):
        ...
        return self.check_crosstable_aggregate(
            'char_dir_values', 'galois_orbit', 'orbit_label', 'conrey_index')
    ...
    def check_parity_value(self):
        ...
        return (
            self._run_crosstable(
                ...
                constraint={
                    'parity': -1},
                subselect_wrapper="ALL") + self._run_crosstable(
                SQL("t2.values[1][2]"),
                'char_dir_values',
                0,
                'orbit_label',
    ...
    def check_order_parity(self, rec, verbose=False):
        ...
        success = (parity == rec['parity'] and char.conductor(
        ) == rec['conductor'] and char.multiplicative_order() == rec['order'])
        if verbose and not success:
            print(
                "Order-parity failure",
                parity,
                rec['parity'],
                char.conductor(),
                rec['conductor'],
                char.multiplicative_order(),
                rec['order'])
        return success
