...
def Sp4Z_j_space(k, j):
    ...
    return render_template(
        'ModularForm_GSp4_Q_full_level_space.html',
...
def render_family_page(family, args, bread):
    ...
    return render_template(
        "ModularForm_GSp4_Q_family.html",
        title='Siegel modular forms for $' +
        family.latex_name +
        '$',
        bread=bread,
        info=info)
...
def render_search_results_page(args, bread):
    ...
    return render_template(
        "ModularForm_GSp4_Q_search_results.html",
...
def render_sample_page(family, sam, args, bread):
    ...
    try:
        ...
        evs_to_show = []
        fcs_to_show = []
    info['evs_to_show'] = sorted([n for n in (evs_to_show if len(
        evs_to_show) else sam.available_eigenvalues()[:10])])
    info['fcs_to_show'] = sorted([n for n in (fcs_to_show if len(
        fcs_to_show) else sam.available_Fourier_coefficients()[1:6])])
    ...
    return render_template(
        "ModularForm_GSp4_Q_sample.html",
        title=title,
        bread=bread,
