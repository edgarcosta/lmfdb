...
def _dimension_Gamma_2(wt_range, j, group='Gamma(2)'):
    ...
    latex_names = {
        'Gamma(2)': r'\Gamma(2)',
        'Gamma0(2)': r'\Gamma_0(2)',
        'Gamma1(2)': r'\Gamma_1(2)',
        'Sp4(Z)': r'\mathrm{Sp}(4,\mathbb{Z})'}

    if is_odd(j):
        dct = dict((k, dict((h, [0, 0, 0])
                            for h in partitions)) for k in wt_range)
        for k in dct:
            dct[k]['All'] = [0, 0, 0]
    ...
    if 'Gamma(2)' == group:
        dct = dict((k,
                    dict((p,
                          [total[p][k],
                           total[p][k] - cusp[p][k],
                              cusp[p][k]]) for p in partitions)) for k in wt_range)
        for k in dct:
