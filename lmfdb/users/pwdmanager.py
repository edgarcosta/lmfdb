...
class PostgresUserTable(PostgresBase):
    ...
    def new_user(self, uid, pwd=None, full_name=None, about=None, url=None):
        ...
        insertor = SQL(
            u"INSERT INTO userdb.users (username, bcpassword, created, full_name, about, url) VALUES (%s, %s, %s, %s, %s, %s)")
        self._execute(
            insertor, [
    ...
    def lookup(self, uid):
        ...
        return {
            ...
            value in zip(
                self._cols,
                cur.fetchone()) if value is not None}
    ...
    def full_names(self, uids):
        ...
        return [{k: v for k, v in zip(
            ["username", "full_name"], rec)} for rec in cur]
...
class LmfdbUser(UserMixin):
    ...
    def is_anonymous(self):
        """required by flask-login user class"""
        if StrictVersion(FLASK_LOGIN_VERSION) < StrictVersion(
                FLASK_LOGIN_LIMIT):
