...
def get_bread(breads=[]):
    bc = [('Representations', "/Representation"),
          ("mod &#x2113;", url_for(".index"))]
    for b in breads:
        bc.append(b)
    return bc
...
def rep_galois_modl_by_label_or_name(lab):
    ...
    if rep_galois_modl_label_regex.match(lab):
        flash_error(
            "The integral rep_galois_modl %s is not recorded in the database or the label is invalid",
            lab)
    else:
        flash_error(
            "No integral rep_galois_modl in the database has label or name %s",
            lab)
...
def download_search(info):
    ...
    s = '\n'
    s += c + \
        ' Integral rep_galois_modls downloaded from the LMFDB on %s. Found %s rep_galois_modls.\n\n' % (mydate, len(res))
...
@search_wrap(
    template="rep_galois_modl-search.html",
    ...
    shortcuts={
        ...
        'label': lambda info: rep_galois_modl_by_label_or_name(
            info.get('label'))},
    projection=[
        'label',
        'dim',
...
def render_rep_galois_modl_webpage(**args):
    ...
    if data is None:
        ...
        bread = [('Representations', "/Representation"),
                 ("mod &#x2113;", url_for(".rep_galois_modl_render_webpage"))]
        flash_error(
            "%s is not a valid label for a mod &#x2113; Galois representation in the database.",
            lab)
        return render_template(
            "rep_galois_modl-error.html",
            title=t,
            properties=[],
            bread=bread,
            learnmore=learnmore_list())
    ...
    bread = [
        ('Representations',
         "/Representation"),
        ("mod &#x2113;",
         url_for(".rep_galois_modl_render_webpage")),
        ('%s' %
         data['label'],
         ' ')]
    credit = rep_galois_modl_credit
    ...
    if info['field_deg'] > int(1):
        try:
            pol = str(
                ...
                "")
            info['field_str'] = str(
                r'$\mathbb{F}_%s \cong \mathbb{F}_%s[a]$ where $a$ satisfies: $%s=0$' %
                (str(
                    data['field_char']), str(
                    data['field_char']), pol))
        except Exception:
    ...
    info['len_good'] = [int(i + 1)
                        for i in range(len(data['good_prime_list'][0][1]))]
    for n in data['good_prime_list']:
        try:
            n1 = [int(n[0]), [str(m) for m in n[1]],
                  str(n[2]), int(n[3]), int(n[4])]
        except Exception:
            n1 = [int(n[0]), [str(m) for m in n[1]],
        ...
        info['download_list'] = [
            (i,
             url_for(
                 ".render_rep_galois_modl_webpage_download",
                 label=info['label'],
                 lang=i)) for i in [
                'gp',
                'magma',
...
def completeness_page():
    ...
    bread = [('Representations', "/Representation"), ("mod &#x2113;",
                                                      url_for(".rep_galois_modl_render_webpage")), ('Completeness', '')]
    credit = rep_galois_modl_credit
    return render_template(
        "single.html",
        kid='dq.rep_galois_modl.extent',
        credit=credit,
        title=t,
        bread=bread,
        learnmore=learnmore_list_remove('Completeness'))
...
def how_computed_page():
    ...
    bread = [('Representations', "/Representation"), ("mod &#x2113;",
                                                      url_for(".rep_galois_modl_render_webpage")), ('Source', '')]
    credit = rep_galois_modl_credit
    return render_template(
        "single.html",
        kid='dq.rep_galois_modl.source',
        credit=credit,
        title=t,
        bread=bread,
...
def labels_page():
    ...
    bread = [('Representations', "/Representation"), ("mod &#x2113;",
                                                      url_for(".rep_galois_modl_render_webpage")), ('Labels', '')]
    credit = rep_galois_modl_credit
    return render_template(
        "single.html",
        kid='rep_galois_modl.label',
        credit=credit,
        title=t,
        bread=bread,
