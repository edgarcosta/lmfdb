...
class totaler(object):
    ...
    def __init__(
            ...
            col_counts=True,
            col_proportions=True,
            corner_count=None,
            corner_proportion=None,
            include_links=True,
...
class StatsDisplay(UniqueRepresentation):
    ...
    def display_data(
        ...
        if len(cols) == 1:
            ...
            if show_total:
                total = {
                    'count': total, 'query': "{0}{1}".format(
                        base_url, cols[0]), 'proportion': _format_percentage(
                        total, self._overall, show_zero=True)}
    ...
    def prep(self, attr):
        ...
        if not isinstance(top_title, str):
            ...
            attr['top_title'] = joiner.join(
                (display_knowl(
                    knowl,
                    title=title) if knowl else title) for title,
                knowl in top_title)
