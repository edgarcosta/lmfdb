...
# parse_newton_polygon and parse_abvar_decomp are defined in
...
def prep_raw(inp, names={}):
    ...
    inp = implicit_mul(
        inp,
        level=10)  # level = 10 includes (a+b)(c+d) -> (a+b)*(c+d) which isn't safe in Sage but should be okay for us
    ...
    if ',' in arg:
        tmp = [
            parse_range2(
                ...
                parse_singleton,
                parse_endpoint,
                split_minus) for a in arg.split(',')]
...
def parse_range_float(arg, key, exact_den=4, exact_prec=10, mod1=False):
    ...
    if ',' in arg:
        ...
        return [
            key, {'$gte': x - eps(prec), '$lte': x + eps(prec)}], "%s-%s" % (start, end)
...
# Parses signed ints as an int and a sign the fields these are stored are
# passed in as qfield = (sign_field, abs_field)
...
def parse_bracketed_posints(
    ...
    if (not myregex.match(inp) or
        ...
        if exactlength == 2:
            ...
            example = str(list(range(2, exactlength + 2))).replace(" ",
                                                                   "") + " or " + str([3] * exactlength).replace(" ", "")
        elif maxlength is not None:
            lstr = "list of at most %s positive integers" % maxlength
            example = str(list(range(2, maxlength + 2))).replace(" ", "") + \
                " or " + str([2] * max(1, maxlength - 2)).replace(" ", "")
        else:
...
def parse_bracketed_rats(
    ...
    if (not BRACKETED_RAT_RE.match(inp) or
        ...
        if exactlength == 2:
            ...
            example = str(list(range(2, exactlength + 2))).replace(", ",
                                                                   "/13,") + " or " + str([3] * exactlength).replace(", ", "/4,")
        elif maxlength is not None:
            lstr = "list of at most %s rational numbers" % maxlength
            example = str(list(range(2, maxlength + 2))).replace(", ", "/13,") + \
                " or " + str([2] * max(1, maxlength - 2)).replace(", ", "/41,")
        else:
...
@search_parser(clean_info=True,
               ...
               error_is_safe=True)  # see SearchParser.__call__ for actual arguments when calling
...
def parse_bool(inp, query, qfield, process=None, blank=[]):
    ...
    if inp in [
        ...
        "yes",
        "1",
            "even"]:  # artin reps use parse_bool for an is_even parity field
...
def parse_paired_fields(
    ...
    def combine(D1, D2):
        ...
        if not L1:
            ...
            return [{A.keys()[0]:A.values()[0], B.keys()[0]:B.values()[0]}
    ...
    def make_sub_query(part):
        ...
        if part[0] == '[':
            ...
            if parse_singleton is str:
                all_operand = [val for val in parsed_values if isinstance(
                    val, parse_singleton) and '-' not in val and ',' not in val]
            else:
                all_operand = [val for val in parsed_values
                               if isinstance(val, parse_singleton)]
            ...
            elemMatch_operand = [
                val for val in parsed_values if not isinstance(
                    val, parse_singleton) and isinstance(
                    val, dict)]
...
def parse_string_start(
    ...
    def make_sub_query(part):
        ...
        if part[0] == '[':
            ...
            if '-' in part[1:]:
                raise SearchParsingError("Ranges not supported.")
            sub_query[qfield] = {'$startswith': '%s %s ' % (
