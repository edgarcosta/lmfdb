...
def half_integral_weight_form_render_webpage():
    ...
    if len(args) == 0:
        ...
        return render_template(
            "half_integral_weight_form_all.html",
            info=info,
            credit=credit,
            title=t,
            bread=bread)
...
@search_wrap(
    template="half_integral_weight_form_search.html",
    ...
    projection=[
        ...
        'dim'],
    cleaners={
        'char': lambda v: r"\chi_{" + v['character'].split(".")[0] + "}(" + v['character'].split(".")[1] + r",\cdot)",
...
def render_hiwf_webpage(**args):
    ...
    return render_template(
        "half_integral_weight_form.html",
        info=info,
        credit=credit,
        title=t,
        bread=bread)
