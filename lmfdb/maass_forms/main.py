...
def bread_prefix(): return [('Modular forms', url_for(
    'modular_forms')), ('Maass', url_for('.index'))]
...
def index():
    ...
    return render_template(
        ...
        bread=bread,
        dbcount=db.maass_newforms.count())
    ...
    def __init__(self):
        ...
        symmetry = SelectBox(
            name="symmetry", label="Symmetry", knowl="mf.maass.mwf.symmetry", options=[
                ("", "any symmetry"), ("1", "even only"), ("-1", "odd only")])
...
@search_wrap(template="maass_search_results.html",
             ...
             cleaners={"character_link": lambda v: character_link(v['level'],
                                                                  v['conrey_index']),
                       "symmetry_pretty": lambda v: symmetry_pretty(v['symmetry']),
                       "fricke_pretty": lambda v: fricke_pretty(v['fricke_eigenvalue']),
                       "spectral_link": lambda v: '<a href="' + url_for('.by_label',
                                                                        label=v['maass_id']) + '">' + str(v['spectral_parameter']) + '</a>',
                       },
             bread=lambda: bread_prefix() + [('Search results',
                                              '')],
             learnmore=learnmore_list,
             url_for_label=lambda label: url_for(".by_label",
                                                 label=label),
             )
    ...
    for v in ['rows', 'cols']:
        if info.get('edit_' + v):
            if not re.match(r"^[1-9][0-9]*$", info['edit_' + v]):
                flash_error(
                    "%s is not a valid input for %s, it needs to be a positive integer.", info['edit_' + v], v)
            else:
