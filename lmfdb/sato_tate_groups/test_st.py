...
class SatoTateGroupTest(LmfdbTest):
    ...
    def test_by_label(self):
        ...
        assert '0.1.37' in L.get_data(
            ...
            as_text=True)
        L = self.tc.get(
            '/SatoTateGroup/?label=0.1.mu(37)',
            follow_redirects=True)
        assert '0.1.37' in L.get_data(
            as_text=True) and 'mu(185)' in L.get_data(
            as_text=True)
        L = self.tc.get(
            '/SatoTateGroup/?label=0.1.mu(100000000000000000001)',
            follow_redirects=True)
    ...
    def test_completeness(self):
        ...
        data = list(db.gps_st.search(
            {'weight': int(1), 'degree': int(2)}, projection='label'))
        assert len(data) == 3
        print("")
        for label in data:
        ...
        data = list(db.gps_st.search(
            {'weight': int(1), 'degree': int(4)}, projection='label'))
        assert len(data) == 52
        ...
        for label in data:
            ...
            assert label in L.get_data(
                ...
                as_text=True)
        L = self.tc.get(
            '/SatoTateGroup/?components=999999999&include_irrational=yes')
        assert 'unique match' in L.get_data(
            as_text=True) and 'mu(999999999)' in L.get_data(
            as_text=True)
