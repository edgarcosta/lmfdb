...
def decimal_pretty(
        s,
        min_chars_before_decimal=1,
        max_chars_after_decimal=6,
        max_chars=10):
...
def end_statement(factorsQQ, factorsRR, field='', ring=None):
    ...
    if ring:
        ...
        if ring[0] == 1:
            ...
            # and we can prettify them
                ...
                statement += r'\(' + r' \times '.join(
                    [ring_pretty(factorQQ[1], 1) for factorQQ in factorsQQ]) + r'\)'
        ...
        # can describe the maximal order here
            ...
            statement += r"an order of index \(%s\) in \(%s\)" % (ring[0], r' \times '.join(
    ...
    # If there are two factors, then we get two at most quadratic fields:
        ...
        statement += (
            r"<a href=%s>%s</a> \(\times\) <a href=%s>%s</a>" %
            (url_for(
                "number_fields.by_label",
                label=factorsQQ[0][0]),
                factorsQQ_pretty[0],
...
def end_lattice_statement(lattice):
    ...
    for ED in lattice:
        ...
        if ED[0][0]:
            ...
            statement += (
                r"Over subfield \(F \simeq \) <a href=%s>%s</a> with generator \(%s\) with minimal polynomial \(%s\)" %
                (url_for(
                    "number_fields.by_label", label=ED[0][0]), field_pretty(
                    ED[0][0]), strlist_to_nfelt(
                    ED[0][2], 'a'), intlist_to_poly(
                    ED[0][1])))
        else:
            statement += (
                r"Over subfield \(F\) with generator \(%s\) with minimal polynomial \(%s\)" %
                (strlist_to_nfelt(
                    ED[0][2], 'a'), intlist_to_poly(
                    ED[0][1])))
        statement += ":\n"
        statement += end_statement(ED[1], ED[2], field='F', ring=ED[3])
        statement += "&nbsp;&nbsp;Sato Tate group: %s" % st_link_by_name(
            1, 4, ED[4])
        statement += "<br>&nbsp;&nbsp;"
...
def split_field_statement(is_simple_geom, field_label, poly):
    if is_simple_geom:
        ...
        url = url_for("number_fields.by_label", label=field_label)
        return (
            r"Splits over the number field \(\Q (b) \simeq \) <a href=%s>%s</a> with defining polynomial:<br>&nbsp;&nbsp;\(%s\)" %
            (url, pretty, poly))
    else:
    ...
    for n in range(len(coeffs)):
        ...
        # Otherwise give defining equation:
            ...
            statement += r"<br>&nbsp;&nbsp;\(g_4 = %s\)<br>&nbsp;&nbsp;\(g_6 = %s\)" % tuple(
                map(lambda x: strlist_to_nfelt(x, 'b'), coeffs[n]))
...
def mw_gens_table(
    invs, gens, hts, pts, comp=PolynomialRing(
        QQ, [
            'x', 'y', 'z'])('y')):
    def divisor_data(P):
    ...
    for i in range(len(invs)):
        ...
        div = (r'2 \cdot' + point_string(D0[0]) if len(D0) == 1 and len(
            Dinf) != 1 else ' + '.join([point_string(P) for P in D0])) if D0 else 'D_0'
        div += ' - '
...
def local_table(N, D, tama, bad_lpolys, cluster_pics):
    ...
    for p in D.prime_divisors():
        ...
        if Cluslist:
            ClusThmb = '<img src="' + \
                Cluslist[0][2] + '" height=19 style="position: relative; top: 50%; transform: translateY(10%);" />'
    ...
    def make_object(self, curve, endo, tama, ratpts, clus, is_curve):
        ...
        if is_curve:
            ...
            if curve['two_torsion_field'][0]:
                ...
                data['two_torsion_field_knowl'] = r"splitting field of \(%s\) with Galois group %s" % (
                    intlist_to_poly(t[1]), group_display_knowl(t[2][0], t[2][1]))
        ...
        if not data['is_simple_geom']:
            ...
            if 'spl_facs_labels' in endo and len(
                    endo['spl_facs_labels']) == len(
                    endo['spl_facs_coeffs']):
        ...
        properties += [('Sato-Tate group',
                       ...
                       (r'\(\End(J) \otimes \Q\)',
                        r'\(%s\)' % data['end_alg_name']),
                       (r'\(\overline{\Q}\)-simple',
                        bool_pretty(data['is_simple_geom'])),
                       (r'\(\mathrm{GL}_2\)-type',
                        bool_pretty(data['is_gl2_type'])),
                       ]
        ...
        # Breadcrumbs
        self.bread = bread = [
            ('Genus 2 curves', url_for(".index")), (r'$\Q$', url_for(".index_Q")), ('%s' %
                                                                                    data['slabel'][0], url_for(
                                                                                        ".by_conductor", cond=data['slabel'][0])), ('%s' %
                                                                                                                                    data['slabel'][1], url_for(
                                                                                                                                        ".by_url_isogeny_class_label", cond=data['slabel'][0], alpha=data['slabel'][1]))]
        if is_curve:
        ...
        code['curve'] = {'sage': 'R.<x> = PolynomialRing(QQ); C = HyperellipticCurve(R(%s), R(%s));' % (
            f, h), 'magma': 'R<x> := PolynomialRing(Rationals()); C := HyperellipticCurve(R!%s, R!%s);' % (f, h)}
        code['simple_curve'] = {
        ...
        if ratpts:
            code['rat_pts'] = {'magma': '[' + ','.join(["C![%s,%s,%s]" % (
