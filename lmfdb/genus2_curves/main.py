...
# doing from sage.interfaces.magma import magma leads to some bugs
...
def by_url_isogeny_class_discriminant(cond, alpha, disc):
    ...
    if not db.g2c_curves.exists({'class': clabel}):
        return abort(
            404,
            'Genus 2 isogeny class %s not found in database.' %
            clabel)
    ...
    data['bread'] = get_bread(
        [
            ('%s' %
             cond, url_for(
                 ".by_conductor", cond=cond)), ('%s' %
                                                alpha, url_for(
                                                    ".by_url_isogeny_class_label", cond=cond, alpha=alpha)), ('%s' %
                                                                                                              disc, url_for(
                                                                                                                  ".by_url_isogeny_class_discriminant", cond=cond, alpha=alpha, disc=disc))])
...
@search_wrap(
    ...
    cleaners={
        "class": lambda v: class_from_curve_label(
            v["label"]),
        "equation_formatted": lambda v: min_eqn_pretty(
            literal_eval(
                v.pop("eqn"))),
        "st_group_link": lambda v: st_link_by_name(
            1,
            4,
            v.pop("st_group")),
    },
    ...
    learnmore=learnmore_list,
    url_for_label=lambda label: url_for(
        ".by_label",
        label=label),
)
...
class G2C_stats(StatsDisplay):
    ...
    formatters = {
        'aut_grp_id': lambda x: aut_grp_dict_pretty.get(
            x,
            x),
        ...
        'st_group': lambda x: st_link_by_name(
            1,
            4,
            x)}
    query_formatters = {
...
class G2CSearchArray(SearchArray):
    ...
    def __init__(self):
        geometric_invariants_type = SelectBox(
            name="geometric_invariants_type", min_width=115, options=[
                ("", "Igusa-Clebsh"), ("igusa_inv", "Igusa"), ("g2_inv", "G2")], )
