...
def download_search(info):
    ...
    s = '\n'
    s += c + \
        ' Integral lattices downloaded from the LMFDB on %s. Found %s lattices.\n\n' % (mydate, len(res))
...
def render_lattice_webpage(**args):
    ...
    if f['shortest'] == "":
        ...
        info['download_shortest'] = [
            (i,
             url_for(
                 ".render_lattice_webpage_download",
                 label=info['label'],
                 lang=i,
                 obj='shortest_vectors')) for i in [
                'gp',
                'magma',
                'sage']]
...
def how_computed_page():
    ...
    return render_template(
        "double.html",
        kid='rcs.source.lattice',
        kid2='rcs.ack.lattice',
        title=t,
        bread=bread,
