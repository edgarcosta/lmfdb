...
class CmfTest(LmfdbTest):
    ...
    def test_download_qexp(self):
        ...
        for label in ['212.2.k.a', '887.2.a.b']:
            ...
            assert 'No q-expansion found for {}'.format(
    ...
    def test_download(self):
        ...
        assert "[7, 31, 35, 43, 51, 55, 59, 63, 67, 71, 79, 87, 91, 115, 139, 227]" in page.get_data(
            as_text=True)
    ...
    def test_expression_level(self):
        ...
        self.check_args(
            '/ModularForm/GL2/Q/holomorphic/?hst=List&level=2*7%5E2&search_type=List',
            '98.2.a.a')
    ...
    def test_download_search(self):
        ...
        page = self.tc.get(
            '/ModularForm/GL2/Q/holomorphic/?Submit=sage&download=1&query=%7B%27level_radical%27%3A+5%2C+%27dim%27%3A+%7B%27%24lte%27%3A+10%2C+%27%24gte%27%3A+1%7D%2C+%27weight%27%3A+10%7D&search_type=List',
            follow_redirects=True)
        assert '5.10.a.a' in page.get_data(as_text=True)
        assert (
            '[5, 10, 1, 2.5751791808193656, [0, 1], "1.1.1.1", [], [], [-8, -114, -625, 4242]]' in page.get_data(
                as_text=True)) or (
            '[5, 10, 1, 2.57517918082, [0, 1], "1.1.1.1", [], [], [-8, -114, -625, 4242]]' in page.get_data(
                as_text=True))  # Different tests for py2 and py3 due to different number of digits being returned.
        ...
        page = self.tc.get(
            '/ModularForm/GL2/Q/holomorphic/?Submit=sage&download=1&query=%7B%27dim%27%3A+%7B%27%24gte%27%3A+2000%7D%2C+%27num_forms%27%3A+%7B%27%24exists%27%3A+True%7D%7D&search_type=SpaceTraces',
            follow_redirects=True)
        assert 'Error: We limit downloads of traces to' in page.get_data(
            as_text=True)
        page = self.tc.get(
            '/ModularForm/GL2/Q/holomorphic/?Submit=sage&download=1&query=%7B%27dim%27%3A+%7B%27%24gte%27%3A+30000%7D%2C+%27num_forms%27%3A+%7B%27%24exists%27%3A+True%7D%7D&search_type=SpaceTraces',
            follow_redirects=True)
    ...
    def test_dimension(self):
        page = self.tc.get(
            '/ModularForm/GL2/Q/holomorphic/?level=10&weight=1-14&dim=1&search_type=List',
            follow_redirects=True)
    ...
    def test_trivial_searches(self):
        ...
        for begin in [
            ...
            for s in Subsets(['has_self_twist=no',
                              'is_self_dual=yes',
                              'nf_label=1.1.1.1',
                              'char_order=1',
                              'inner_twist_count=1']):
        ...
        for begin in [
            ...
            for s in Subsets(['has_self_twist=yes',
                              'has_self_twist=cm',
                              'has_self_twist=rm',
                              'projective_image_type=Dn',
                              'dim=1-4']):
                s = '&'.join(
                    ['/ModularForm/GL2/Q/holomorphic/?search_type=List', begin[0]] + list(s))
                page = self.tc.get(s, follow_redirects=True)
    ...
    def test_parity(self):
        page = self.tc.get(
            '/ModularForm/GL2/Q/holomorphic/?weight_parity=even&char_parity=even&search_type=List')
        assert '11.2.a.a' in page.get_data(as_text=True)
        page = self.tc.get(
            '/ModularForm/GL2/Q/holomorphic/?weight_parity=odd&char_parity=odd&search_type=List')
        assert '23.1.b.a' in page.get_data(as_text=True)
        page = self.tc.get(
            '/ModularForm/GL2/Q/holomorphic/?weight_parity=even&char_parity=even&weight=3&search_type=List')
        assert "No matches" in page.get_data(as_text=True)
        page = self.tc.get(
            '/ModularForm/GL2/Q/holomorphic/?weight_parity=even&char_parity=odd&search_type=List')
    ...
    def test_coefficient_fields(self):
        ...
        page = self.tc.get(
            '/ModularForm/GL2/Q/holomorphic/?level=1-500&weight=2&nf_label=16.0.1048576000000000000.1&prime_quantifier=subsets&search_type=List')
        assert r'\zeta_{40}' in page.get_data(as_text=True)
        assert "Results (6 matches)" in page.get_data(as_text=True)

        page = self.tc.get(
            '/ModularForm/GL2/Q/holomorphic/?level=1-4000&weight=1&nf_label=9.9.16983563041.1&prime_quantifier=subsets&projective_image=D19&search_type=List')
        assert r"Q(\zeta_{38})^+" in page.get_data(as_text=True)
        assert "Results (32 matches)" in page.get_data(as_text=True)

        page = self.tc.get(
            '/ModularForm/GL2/Q/holomorphic/?level=1-100&weight=2&dim=4&nf_label=4.0.576.2&prime_quantifier=subsets&search_type=List')
        assert 'Results (7 matches)' in page.get_data(as_text=True)
        assert r'\Q(\sqrt{2}, \sqrt{-3})' in page.get_data(as_text=True)

        page = self.tc.get(
            '/ModularForm/GL2/Q/holomorphic/?dim=8&char_order=20&cm=no&rm=no&search_type=List')
        assert "Results (17 matches)" in page.get_data(as_text=True)
        assert r"Q(\zeta_{20})" in page.get_data(as_text=True)

        page = self.tc.get(
            '/ModularForm/GL2/Q/holomorphic/?level=1-4000&weight=1&dim=116&search_type=List')
    ...
    def test_selft_twist_disc(self):
        page = self.tc.get(
            '/ModularForm/GL2/Q/holomorphic/?level=1-40&weight=1-6&self_twist_discs=-3&search_type=List')
        for elt in [r'\Q(\sqrt{-39})', r'\Q(\sqrt{-3})']:
            assert elt in page.get_data(as_text=True)
        assert 'Results (22 matches)' in page.get_data(as_text=True)

        page = self.tc.get(
            '/ModularForm/GL2/Q/holomorphic/?level=1-100&self_twist_discs=5&search_type=List')
        for elt in [-55, -11, 5, -5, -1, -95, -19]:
            assert (r'\Q(\sqrt{%d})' % elt) in page.get_data(as_text=True)
        assert 'Results (3 matches)' in page.get_data(as_text=True)
        for d in [3, -5]:
            page = self.tc.get(
                '/ModularForm/GL2/Q/holomorphic/?level=1-100&self_twist_discs=%d&search_type=List' %
                d)
    ...
    def test_AL_search(self):
        ...
        """
        page = self.tc.get(
            '/ModularForm/GL2/Q/holomorphic/?level=15&char_order=1&search_type=List',
            follow_redirects=True)
        assert 'A-L signs' in page.get_data(as_text=True)
        page = self.tc.get(
    ...
    def test_Fricke_signs_search(self):
        ...
        """
        page = self.tc.get(
            '/ModularForm/GL2/Q/holomorphic/?level=15%2C20&weight=2&dim=1&search_type=List',
            follow_redirects=True)
        assert 'Fricke sign' in page.get_data(as_text=True)
        page = self.tc.get(
            '/ModularForm/GL2/Q/holomorphic/?char_order=1&search_type=List',
            follow_redirects=True)
        assert 'Fricke sign' in page.get_data(as_text=True)
    ...
    def test_is_self_dual(self):
        page = self.tc.get(
            '/ModularForm/GL2/Q/holomorphic/?is_self_dual=yes&search_type=List',
            follow_redirects=True)
        for elt in ['23.1.b.a', '31.1.b.a', '111.1.d.a']:
            assert elt in page.get_data(as_text=True)
        page = self.tc.get(
            '/ModularForm/GL2/Q/holomorphic/?is_self_dual=no&search_type=List',
            follow_redirects=True)
    ...
    def test_hecke_charpolys(self):
        ...
        test_data = {  # Dimension 1
            ...
            '/294/5/b/f': {2: r'\( ( 8 + T^{2} )^{5} \)',
                           # The following test checks that monomials do not
                           # have superfluous parentheses
                           7: r'\( T^{10} \)'},
        }
        ...
        for label, some_expected_charpolys in test_data.items():
            ...
            for p, expected_pth_charpoly in some_expected_charpolys.items():
                assert charpoly_test_string.format(
                    p, expected_pth_charpoly) in page_as_text
