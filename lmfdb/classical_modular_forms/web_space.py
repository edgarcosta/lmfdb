...
def newform_search_link(text, title=None, **kwd):
    ...
    return "<a href='%s'%s>%s</a>" % (link,
                                      "" if title is None else " title='%s'" % title,
                                      text)
...
def ALdim_table(al_dims, level, weight):
    ...
    for i, (vec, dim, cnt) in enumerate(al_dims):
        ...
        if len(vec) > 1:
            row.append(
                r"<td class='right'>\(%s\)</td>" %
                ('+' if sign == 1 else '-'))
        query = {
            'level': level,
            'weight': weight,
            'char_order': 1,
            'atkin_lehner_string': s}
        if cnt == 1:
            query['jump'] = 'yes'
    ...
    if num_primes > 1:
        ...
        minus_link = newform_search_link(
            r'\(%s\)' % fricke[-1], level=level, weight=weight, char_order=1, fricke_eigenval=-1)
        rows.append(
            r"<tr><td colspan='%s'>%s</td><td class='right'>\(+\)</td><td>%s</td></tr>" %
            (num_primes, plus_knowl, plus_link))
        rows.append(
            r"<tr><td colspan='%s'>%s</td><td class='right'>\(-\)</td><td>%s</td></tr>" %
            (num_primes, minus_knowl, minus_link))
    return (
        "<table class='ntdata'><thead><tr>%s</tr></thead><tbody>%s</tbody></table>" %
...
def convert_spacelabel_from_conrey(spacelabel_conrey):
    ...
    return db.mf_newspaces.lucky(
        {'conrey_indexes': {'$contains': chi}, 'level': N, 'weight': k}, projection='label')


def trace_expansion_generic(space, prec_max=10):
    prec = min(len(space.traces) + 1, prec_max)
    return web_latex(coeff_to_power_series(
        [0] + space.traces[:prec - 1], prec=prec), enclose=True)
...
class DimGrid():
    ...
    def __add__(self, other):
        if isinstance(
                other,
                int) and other == 0:  # So that we can do sum(grids)
            return self
...
class WebNewformSpace():
    def __init__(self, data):
        ...
        oldspaces = db.mf_subspaces.search({'label': self.label, 'sub_level': {'$ne': self.level}}, [
                                           'sub_level', 'sub_char_orbit_index', 'sub_conrey_indexes', 'sub_mult'])
        self.oldspaces = [
            (old['sub_level'],
             old['sub_char_orbit_index'],
                old['sub_conrey_indexes'][0],
        ...
        self.properties += [
            ('Level', prop_int_pretty(
                self.level)), ('Weight', prop_int_pretty(
                    self.weight)), ('Character orbit', '%s.%s' %
                                    (self.level, self.char_orbit_label)), ('Rep. character', '$%s$' %
                                                                           self.char_conrey_str), ('Character field', r'$\Q%s$' %
                                                                                                   ('' if self.char_degree == 1 else r'(\zeta_{%s})' %
                                                                                                    self.char_order)), ('Dimension', prop_int_pretty(
                                                                                                        self.dim)), ]
        ...
        self.downloads = [
            ('Trace form to text', url_for(
                'cmf.download_traces', label=self.label)), ('All stored data to text', url_for(
                    '.download_newspace', label=self.label)), ('Underlying data', url_for(
                        '.mf_data', label=self.label)), ]
...
class WebGamma1Space():
    def __init__(self, level, weight):
        ...
        oldspaces = db.mf_gamma1_subspaces.search({'level': level, 'sub_level': {
                                                  '$ne': level}, 'weight': weight}, ['sub_level', 'sub_mult'])
        self.oldspaces = [(old['sub_level'], old['sub_mult'])
                          for old in oldspaces]
        self.dim_grid = sum(DimGrid.from_db(space)
        ...
        self.old_dim = sum((space['cusp_dim'] - space['dim'])
        ...
        self.downloads = [
            ('Trace form to text', url_for(
                'cmf.download_traces', label=self.label)), ('All stored data to text', url_for(
                    'cmf.download_full_space', label=self.label)), ('Underlying data', url_for(
                        '.mf_data', label=self.label)), ]
    ...
    def decomposition(self):
        ...
        for i, (space, forms) in enumerate(self.decomp):
            ...
            chi_str = r"\chi_{%s}(%s, \cdot)" % (
                space['level'], space['conrey_indexes'][0])
            chi_rep = '<a href="' + url_for(
