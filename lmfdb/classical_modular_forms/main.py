...
def ALdims_knowl(al_dims, level, weight):
    ...
    short = "+".join(r'\(%s\)' %
                     dim_dict.get(vec, 0) for vec in cartesian_product_iterator([[1, -
                                                                                  1] for _ in range(len(al_dims[0][0]))]))
...
def by_url_newform_conrey5(
    ...
    return redirect(
        url_for(
            "cmf.by_url_embedded_newform_label",
            level=level,
            weight=weight,
            char_orbit_label=char_orbit_label,
            hecke_orbit=hecke_orbit,
            conrey_index=conrey_index,
            embedding=embedding),
        code=301)
...
def parse_character(inp, query, qfield, prim=False):
    ...
    if orbit.isalpha():
        ...
        if prim:
            ...
            conductor = db.char_dir_orbits.lucky(
                {'modulus': level, 'orbit_index': orbit}, 'conductor')
...
def newspace_parse(info, query):
    ...
    if info['search_type'] != 'SpaceDimensions':
        ...
        if 'num_forms' not in query and info.get('all_spaces') != 'yes':
            ...
            # newforms (Nk2 > 4000, nontrivial character)
...
def _AL_col(i, p):
    return ProcessedCol(
        ...
        None,
        str(p),
        lambda evs: "+" if evs[i][1] == 1 else "-",
        orig="atkin_lehner_eigenvals",
        align="center",
        mathmode=True,
        default=True)


newform_columns = SearchColumns([LinkCol("label",
                                         "cmf.label",
                                         "Label",
                                         url_for_label,
                                         default=True),
                                 ...
                                 MultiProcessedCol("character",
                                                   ...
                                                   ["level",
                                                    "char_orbit_label"],
                                                   lambda level,
                                                   orb: display_knowl('character.dirichlet.orbit_data',
                                                                      title=f"{level}.{orb}",
                                                                      kwargs={"label": f"{level}.{orb}"}),
                                                   short_title="character"),
                                 MultiProcessedCol("prim",
                                                   "character.dirichlet.primitive",
                                                   "Prim",
                                                   ["char_conductor",
                                                    "prim_orbit_index"],
                                                   lambda cond,
                                                   ind: display_knowl('character.dirichlet.orbit_data',
                                                                      title=f"{cond}.{num2letters(ind)}",
                                                                      kwargs={"label": f"{cond}.{num2letters(ind)}"}),
                                 ...
                                 MultiProcessedCol("field",
                                                   ...
                                                   nf_link,
                                                   default=True),
                                 ProcessedCol("projective_image",
                                              "cmf.projective_image",
                                              "Image",
                                              lambda img: ('' if img == '?' else '$%s_{%s}$' % (img[:1],
                                                                                                img[1:])),
                                              ...
                                              align="center",
                                              short_title="projective image"),
                                 MultiProcessedCol("cm",
                                                   "cmf.self_twist",
                                                   "CM",
                                                   ["is_cm",
                                                    "cm_discs"],
                                                   lambda is_cm,
                                                   cm_discs: ", ".join(map(quad_field_knowl,
                                                                           cm_discs)) if is_cm else "None",
                                                   short_title="CM",
                                                   default=True),
                                 MultiProcessedCol("rm",
                                                   "cmf.self_twist",
                                                   "RM",
                                                   ["is_rm",
                                                    "rm_discs"],
                                                   lambda is_rm,
                                                   rm_discs: ", ".join(map(quad_field_knowl,
                                                                           rm_discs)) if is_rm else "None",
                                                   contingent=lambda info: any(mf.get('weight') == 1 for mf in info["results"]),
                                                   short_title="RM",
                                                   default=True),
                                 CheckCol("is_self_dual",
                                          "cmf.selfdual",
                                          "Self-dual"),
                                 MathCol("inner_twist_count",
                                         "cmf.inner_twist_count",
                                         "Inner twists"),
                                 MathCol("analytic_rank",
                                         "cmf.analytic_rank",
                                         "Rank*"),
                                 ColGroup("traces",
                                          "cmf.trace_form",
                                          ...
                                          default=True),
                                 SpacerCol("atkin_lehner",
                                           contingent=display_AL,
                                           default=True),
                                 ColGroup("atkin_lehner",
                                          "cmf.atkin-lehner",
                                          "A-L signs",
                                          lambda info: [_AL_col(i,
                                                                pair[0]) for i,
                                                        pair in enumerate(info["results"][0]["atkin_lehner_eigenvals"])],
                                          contingent=display_AL,
                                          default=True,
                                          orig=["atkin_lehner_eigenvals"]),
                                 ProcessedCol("fricke_eigenval",
                                              "cmf.fricke",
                                              "Fricke sign",
                                              lambda ev: "$+$" if ev == 1 else ("$-$" if ev else ""),
                                              contingent=display_Fricke,
                                              default=lambda info: not display_AL(info),
                                              align="center"),
                                 ProcessedCol("hecke_ring_index_factorization",
                                              "cmf.coefficient_ring",
                                              "Coefficient ring index",
                                              lambda fac: "" if fac == "" else factor_base_factorization_latex(fac),
                                              mathmode=True,
                                              align="center"),
                                 ProcessedCol("sato_tate_group",
                                              "cmf.sato_tate",
                                              "Sato-Tate",
                                              st_display_knowl,
                                              short_title="Sato-Tate group"),
                                 MultiProcessedCol("qexp",
                                                   "cmf.q-expansion",
                                                   "$q$-expansion",
                                                   ["label",
                                                    "qexp_display"],
                                                   lambda label,
                                                   disp: fr'<a href="{url_for_label(label)}">\({disp}\)</a>' if disp else "",
                                                   default=True)],
                                ['analytic_conductor',
                                 'analytic_rank',
...
def dimension_space_postprocess(res, info, query):
    ...
    if 'char_orbit_index' in query or 'prim_orbit_index' in query:
        ...
        def url_generator(N, k):
            return url_for(
                ".by_url_full_gammma1_space_label",
                level=N,
                weight=k)
    ...
    dimension_common_postprocess(
        info, query, [
            'M', 'S', 'E'], [
            'all', 'new', 'old'], url_generator, pick_table, switch_text)
    dim_dict = {}
    for space in res:
        N = space['level']
        k = space['weight']
    ...
    [
        ...
        MultiProcessedCol(
            ...
            [
                ...
                "conrey_indexes"],
            lambda level,
            indexes: r'<a href="%s">\( \chi_{%s}(%s, \cdot) \)</a>' %
            (url_for(
        ...
        MultiProcessedCol(
            ...
            [
                ...
                "AL_dims"],
            display_ALdims,
            contingent=show_ALdims_col,
            default=True,
            short_title="Atkin-Lehner dimensions",
            align="center",
            td_class=" nowrap")])
...
class CMF_stats(StatsDisplay):
    ...
    def short_summary(self):
        return r'The database currently contains %s (Galois orbits of) %s, corresponding to %s modular forms over the complex numbers.  You can <a href="%s">browse further statistics</a> or <a href="%s">create your own</a>.' % (
            self.nforms, self.newform_knowl, self.ndim, url_for(".statistics"), url_for(".dynamic_statistics"))

    @property
    def summary(self):
        return r"The database currently contains %s (Galois orbits of) %s and %s nonzero %s, corresponding to %s modular forms over the complex numbers.  In addition to the statistics below, you can also <a href='%s'>create your own</a>." % (
            self.nforms, self.newform_knowl, self.nspaces, self.newspace_knowl, self.ndim, url_for(".dynamic_statistics"))
