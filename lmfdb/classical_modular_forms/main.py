...
def set_info_funcs(info):
    ...
    def nf_link(mf):
        ...
        if m and d != 2:
            ...
            return field_display_gen(
                mf.get('nf_label'),
                mf.get('field_poly'),
                mf.get('field_disc_factorization'),
                truncate=16)
    ...
    info["display_projective_image"] = lambda mf: ('%s_{%s}' % (
        mf['projective_image'][:1], mf['projective_image'][1:])) if 'projective_image' in mf else ''
    ...
    def display_decomp(space):
        ...
        for dim in sorted(dim_dict.keys()):
            count = dim_dict[dim]
            query = {'weight': space['weight'], 'char_label': '%s.%s' % (
                space['level'], space['char_orbit_label']), 'dim': dim}
            if count > 3:
    ...
    info['show_ALdims_col'] = lambda spaces: any(
        space.get('AL_dims') for space in spaces)
