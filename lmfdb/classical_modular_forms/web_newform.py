...
class WebNewform(object):
    def __init__(
        ...
        if self.embedding_label is None:
            ...
            char_values = db.char_dir_values.lucky({'label': '%s.%s' % (
                self.level, self.char_conrey)}, ['order', 'values_gens'])
            if char_values is None:
                raise ValueError("Invalid Conrey label")
            # [[i,[[1, m]]] for i, m in char_values['values_gens']]
            self.hecke_ring_character_values = char_values['values_gens']
        ...
        if self.embedding_label is None:
            self.twists = [r for r in db.mf_twists_nf.search(
                {'source_label': self.label})]
        else:
            self.embedded_twists = [r for r in db.mf_twists_cc.search(
        ...
        if self.is_cm and self.is_rm:
            ...
            disc = ' and '.join([str(d)
                                 for d in self.self_twist_discs if d < 0])
    ...
    def bread(self):
        kwds = dict(
            ...
            hecke_orbit=cremona_letter_code(
                self.hecke_orbit - 1))
    ...
    def friends(self):
        ...
        if self.sato_tate_group:
            res.append(
                (r'Sato-Tate group \({}\)'.format(
                    get_name(
                        self.sato_tate_group)[0]),
                    '/SatoTateGroup/' +
                    self.sato_tate_group))
        if self.embedding_label is not None:
        ...
        if self.weight <= 200:
            ...
            if self.embedding_label is None and len(
                    self.conrey_indexes) * self.rel_dim > 50:
                ...
                return '<script id="properties_script">$( document ).ready(function() {properties_lfun(%r, %r, %r, %r, %r)}); </script>' % (
                    res, str(self.label), str(nf_url), self.conrey_indexes, self.rel_dim)
            if self.dim > 1:
        ...
        if self.embedding_label is None:
            ...
            downloads.append(
                ('All stored data to text', url_for(
                    '.download_newform', label=self.label)))
        else:
            downloads.append(
                ('Coefficient data to text', url_for(
                    '.download_embedded_newform', label='%s.%s' %
                    (self.label, self.embedding_label))))
        return downloads
        ...
        if not cc_data:
            ...
            if format in angles_formats:
                ...
                chars = [ConreyCharacter(self.level, char)
                         for char in self.conrey_indexes]
    ...
    def projective_image_knowl(self):
        if self.projective_image:
            ...
            return gp_display if gp_label is None else small_group_label_display_knowl(
                gp_label, gp_display)
    ...
    def display_hecke_cutters(self):
        ...
        desc = "<p>This %s can be constructed as the " % (
            display_knowl('cmf.newform_subspace', 'newform subspace'))
        if len(polynomials) > 1:
            ...
            desc = r"<p>This %s is the entire %s %s.</p> " % (display_knowl(
                'cmf.newform_subspace',
                'newform subspace'),
                display_knowl(
                'cmf.newspace',
                'newspace'),
                self.display_newspace())
        return desc
    ...
    def _make_frac(self, num, den):
        ...
        if den == 1:
            ...
            if paren:
                return r"\((\)%s\()/%s\)" % (num,
                                             make_bigint(web_latex(den, enclose=False)))
            else:
                return r"%s\(/%s\)" % (num,
                                       make_bigint(
                                           web_latex(
...
<div class="toggle">
  <p><a onclick="switch_basis('forward-basis'); return false" href='#'>Display \(\beta_i\) in terms of \(%s^j\)</a></p>
        ...
        if len(self.hecke_ring_numerators) > 3 and forward_size > 240 and 2 * \
            ...
            return html % (" nodisplay",
                           self._order_basis_forward(),
                           self._nu_latex,
                           "",
                           self._order_basis_inverse(),
                           self._nu_latex)
        else:
            return html % ("",
                           self._order_basis_forward(),
    ...
    def display_twists(self):
        ...
        twists1 = [
            ...
            '<thead>',
            '<tr><th colspan=8>&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;By %s</th></tr>' %
            display_knowl(
                'cmf.twist',
                'twisting character orbit'),
            '<tr>',
            th_wrap(
        ...
        for r in sorted(
            ...
            twists1.append('<tr>')
            twists1.extend([td_wrapl(char_link),
                            td_wrapl(parity_text(r['parity'])),
                            td_wrapr(r['order']),
                            td_wrapr(r['multiplicity']),
                            td_wrapl(twist_type(r)),
                            td_wrapl(target_link),
                            td_wrapc(minimality),
                            td_wrapr(r['target_dim'])])
            twists1.append('</tr>')
        ...
        twists2 = [
            '<table class="ntdata" style="float: left">',
            '<thead>',
            '<tr><th colspan=8>&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;By %s</th></tr>' %
            display_knowl(
                'cmf.twist',
                'twisted newform orbit'),
            '<tr>',
            th_wrap(
                'cmf.twist',
                'Twist'),
        ...
        for r in sorted(
            ...
            twists2.extend(
                [
                    td_wrapl(target_link), td_wrapc(minimality), td_wrapr(
                        r['target_dim']), td_wrapl(char_link), td_wrapl(
                        parity_text(
                            r['parity'])), td_wrapr(
                        r['order']), td_wrapr(
                            r['multiplicity']), td_wrapl(
                                twist_type(r))])
            twists2.append('</tr>')
    ...
    def display_embedded_twists(self):
        ...
        for r in sorted(
            self.embedded_twists,
            key=lambda x: [
                x['conductor'],
                x['twisting_conrey_index'],
                revcode(
                    x['target_hecke_orbit_code']),
                x['target_conrey_index'],
                x['target_embedding_index']]):
        ...
        for r in sorted(
            ...
            target_link = '<a href="%s">%s</a>' % (
                '/ModularForm/GL2/Q/holomorphic/' + r['target_label'].replace('.', '/'), r['target_label'])
            twists2.append('<tr>')
            twists2.extend([td_wrapl(target_link),
                            td_wrapc(minimality),
                            td_wrapr(r['target_dim']),
                            td_wrapl(char_link),
                            td_wrapl(parity_text(r['parity'])),
    ...
    def eigs_as_seqseq_to_qexp(self, prec_max):
        ...
        # For example, eigs_as_seqseq_to_qexp([[0,0],[1,3]]) returns
        # "\((1+3\beta_{1})q\)\(+O(q^2)\)"
    ...
    def q_expansion(self, prec_max=10):
        ...
        if self.embedding_label:
            ...
            prec = min(self.qexp_prec, prec_max)
            if self.dim == 1:
                s = web_latex(coeff_to_power_series(
                    [self.qexp[n][0] for n in range(prec)], prec=prec), enclose=True)
            else:
    ...
    def trace_expansion(self, prec_max=10):
        ...
        s = web_latex(coeff_to_power_series(
            self.texp[:prec], prec=prec), enclose=True)
    ...
    def conrey_from_embedding(self, m):
        ...
        return "{c}.{e}".format(
            c=self.cc_data[m]['conrey_index'], e=(
                (m - 1) %
                self.rel_dim) + 1)
    ...
    def embedding(self, m, n=None, prec=6, format='embed'):
        return " ".join([elt(m, n, prec, format) for elt in [
                        self.embedding_re, self.embedding_op, self.embedding_im]])
    ...
    def satake(self, m, p, i, prec=6, format='satake'):
        ...
        """
        if format == 'satake':
            return " ".join([elt(m, p, i, prec) for elt in [
                            self.satake_re, self.satake_op, self.satake_im]])
