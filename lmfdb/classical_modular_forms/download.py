...
class CMF_download(Downloader):
    ...
    def download_qexp(self, label, lang='sage'):
        ...
        code = ''
        hecke_ring_character_values = self.assign(
            lang, 'hecke_ring_character_values', hecke_nf['hecke_ring_character_values'])

        if hecke_nf['hecke_ring_cyclotomic_generator'] > 0:
            func_body = self.get(
                'qexp_function_body_sparse_cyclotomic',
                {}).get(
                lang,
                [])
            ...
            basis_data = ''
            poly_data = self.assign(
                lang, 'poly_data', hecke_nf['hecke_ring_cyclotomic_generator'])
        else:
            ...
            poly_data = self.assign(
                lang, 'poly_data', hecke_nf['field_poly'], level=1)
        ...
        return self._wrap(
            ...
            '.qexp',
            lang=lang,
            title='q-expansion of newform %s,' %
            (label))
    ...
    def download_multiple_traces(self, info, spaces=False):
        ...
        s += 'labels ' + \
            self.assignment_defn[lang] + self.start_and_end[lang][0] + '\\\n'
...
# angles')
    ...
    def download_spaces(self, info):
        ...
        s += c + " %s\n" % proj
        s += 'data ' + self.assignment_defn[lang] + \
            self.start_and_end[lang][0] + '\\\n'
    ...
    def _magma_MakeCharacters(self, newform, hecke_nf):
        ...
        out = [
            '// To make the character of type GrpDrchElt, type "MakeCharacter_%d_%s();"' % (newform.level,
        ...
        if hecke_nf is None or hecke_nf['hecke_ring_character_values'] is None:
            return out + [
                'function MakeCharacter_%d_%s_Hecke(Kf)' %
                (newform.level, newform.char_orbit_label), '    return MakeCharacter_%d_%s();' %
                (newform.level, newform.char_orbit_label), 'end function;']
        else:
            ...
            char_values = [elt[1]
                           for elt in hecke_nf['hecke_ring_character_values']]
            out += [
                '// To make the character of type GrpDrchElt with Codamain the HeckeField, type "MakeCharacter_%d_%s_Hecke();"' % (newform.level,
                                                                                                                                   newform.char_orbit_label),
                'function MakeCharacter_%d_%s_Hecke(Kf)' % (
    ...
    def _magma_qexpCoeffs(self, newform, hecke_nf):
        return [
            ...
            '    chi := MakeCharacter_%d_%s_Hecke(Universe(aps));' %
            (newform.level,
             newform.char_orbit_label),
    ...
    def _magma_MakeNewformModSym(self, newform, hecke_nf):
        ...
        return [
            ...
            '// containing the newform, type "MakeNewformModSym_%s();".' %
            (newform.label.replace(
                ".",
                "_"),
             ),
            '// This may take a long time!  To see verbose output, uncomment the SetVerbose line below.',
            "function MakeNewformModSym_%s()" %
            (newform.label.replace(
                ".",
                "_"),
             ),
            "    R<x> := PolynomialRing(Rationals());",
            "    chi := MakeCharacter_%d_%s();" %
            (N,
             o),
            "    // SetVerbose(\"ModularSymbols\", true);",
            "    Snew := NewSubspace(CuspidalSubspace(ModularSymbols(chi,%d,-1)));" %
            (k,
             ),
            "    Vf := Kernel(%s,Snew);" %
            (cutters,
