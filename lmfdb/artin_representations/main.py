...
def index():
    ...
    if not request.args:
        return render_template(
            "artin-representation-index.html",
...
def artin_representation_jump(info):
    ...
    return redirect(
        url_for(
            ".render_artin_representation_webpage",
            label=label),
        307)
...
@search_wrap(template="artin-representation-search.html",
             ...
             bread=lambda: [('Artin representations',
                             url_for(".index")),
                            ('Search results',
                             ' ')],
...
def artin_representation_search(info, query):
    ...
    parse_restricted(info, query, "frobenius_schur_indicator",
                     qfield="Indicator", allowed=[1, 0, -1], process=int)
...
def search_input_error(info, bread):
    return render_template(
        "artin-representation-search.html",
        req=info,
        title='Artin representation search error',
        bread=bread)
...
def render_artin_representation_webpage(label):
    ...
    if proj_wnf.is_in_db():
        ...
        if proj_coefs != the_nf.polynomial():
            friends.append(
                ("Field {}".format(
                    proj_wnf.get_label()), str(
                    url_for(
                        "number_fields.by_label", label=proj_wnf.get_label()))))
    if case == 'rep':
        ...
        if the_rep.dimension() == 1:
            ...
            if cc.modulus == 1 and cc.number == 1:
                friends.append(
                    ("L-function",
                     url_for(
                         "l_functions.l_function_dirichlet_page",
                         modulus=cc.modulus,
                         number=cc.number)))
            else:
...
class ArtinStats(StatsDisplay):
    ...
    sort_keys = {"GaloisLabel": galdata,
                 ...
                 "Container": galdata}
    formatters = {
        "Conductor": lambda N: scinot(
            range_formatter(N)),
        "Is_Even": lambda x: "even" if x in [
            "even",
            True] else "odd",
    ...
    query_formatters = {
        "Dim": (
            lambda d: "dimension=%s" %
            range_formatter(d)),
        "Conductor": (
            lambda N: "conductor=%s" %
            (unsci(
                range_formatter(N)))),
        "NumBadPrimes": (
            lambda N: "num_ram=%s" %
            range_formatter(N)),
        "GaloisLabel": (
            lambda gal: r"group=%s" %
            (galunformatter(gal))),
        "Proj_nTj": (
            lambda proj: r"projective_image=%s" %
            (galunformatter(proj))),
        "Container": (
            lambda cont: r"container=%s" %
            (galunformatter(cont))),
        "Is_Even": (
            lambda x: r"parity=%s" %
            ("even" if x in [
                ...
                True] else "odd")),
        "Indicator": (
            lambda ind: r"frobenius_schur_indicator=%s" %
            ind),
    }
    buckets = {"Conductor": [trange(a,
                                    b) for (a,
                                            b) in intervals(0,
                                                            8,
                                                            2) + intervals(8,
    ...
    def summary(self):
        return r"The database currently contains {nreps} Galois conjugacy classes of {repknowl}, for a total of {nfields} {nfknowl} with {ngroups} {gpknowl}.  The largest {dimknowl} is ${mdim}$ and the largest {condknowl} is ${mcond} \approx {amcond}$.".format(
            nreps=comma(
                self.nreps), repknowl=display_knowl(
                "artin", "Artin representations"), nfields=comma(
                self.nfields), nfknowl=display_knowl(
                    "artin.number_field", "number fields"), ngroups=self.ngroups, gpknowl=display_knowl(
                        "artin.gg_quotient", "Galois groups"), dimknowl=display_knowl(
                            "artin.dimension", "dimension"), mdim=self.maxdim, condknowl=display_knowl(
                                "artin.conductor", "conductor"), mcond=self.maxcond, amcond=self.amaxcond)
    ...
    def short_summary(self):
        return r'The database currently contains {nreps} Galois conjugacy classes of {repknowl}, for a total of {nfields} {nfknowl}.  Here are some <a href="{url}">further statistics</a>.'.format(
            nreps=comma(
                ...
                "artin", "Artin representations"), nfields=comma(
                self.nfields), nfknowl=display_knowl(
                    "artin.number_field", "number fields"), url=url_for(".statistics"))
