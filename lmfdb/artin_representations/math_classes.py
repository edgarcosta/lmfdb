...
class ArtinRepresentation(object):
    ...
    def conductor_equation(self):
        ...
        if len(factors) == 1 and factors[0][1] == 1:
            ...
            return bigint_knowl(
                self.conductor(),
                sides=3) + r"\(\medspace = " + self.factored_conductor_latex() + r"\)"
    ...
    def factored_conductor_latex(self):
        ...
        tmp = r" \cdot ".join(power_prime(p, val)
    ...
    def projective_field(self):
        ...
        return formatfield(
            projfield, missing_text="Degree %s field" %
            (len(projfield) - 1))
    ...
    def central_char_function(self):
        ...
        localfactorsa = [
            localfactorsa[j] if bad[j] > 0 else -
            1 for j in range(
                len(localfactorsa))]
    ...
    def central_character_as_artin_rep(self):
        ...
        arts = nfgg.ArtinReps()
        # Filter for 1-dim
        arts = [a for a in arts if ArtinRepresentation(
            str(a['Baselabel']) + "c1").dimension() == 1]
    ...
    def number_of_eigenvalues_plus_one_complex_conjugation(self):
        return int(
            Integer(
                self.dimension() +
                self.trace_complex_conjugation()) /
            2)
...
class NumberFieldGaloisGroup(object):
        ...
        if len(x) == 0:
            ...
            if self._data is None:
                ...
                raise AttributeError(
                    "No Galois group data for polynonial %s" %
                    (coeffs))
        self.lowered = False
    ...
    def artin_representations(self):
        return [
            ArtinRepresentation(
                z['Baselabel'],
                z['GalConj']) for z in self.ArtinReps()]
    ...
    def from_cycle_type_to_conjugacy_class_index(self, cycle_type, p):
        ...
        try:
            ...
            raise KeyError(
                "Expecting to find key %s, whose entries have type %s, in %s. For info, keys there have entries of type %s" %
                (cycle_type, type(
                    cycle_type[0]), self._from_cycle_type_to_conjugacy_class_index_dict, type(
                    list(
                        self._from_cycle_type_to_conjugacy_class_index_dict)[0][0])))
        return fn_to_use(p)
