...
def EC_R_plot_zone(f, h):
    ...
    if len(ZF) == 3:
        return plot_zone_union(
            EC_R_plot_zone_piece(
                f, h, ZF[0], ZF[1]), EC_R_plot_zone_piece(
                f, h, ZF[2], 2 * xi - ZF[2]))
...
def EC_R_plot(ainvs, xmin, xmax, ymin, ymax, colour, legend):
    ...
    return implicit_plot(
        ...
        aspect_ratio="automatic",
        color=colour,
        legend_label=legend)  # Add an extra plot outside the visible frame because implicit plots are buggy: their legend does not show (http://trac.sagemath.org/ticket/15903)
...
def EC_nf_plot(K, ainvs, base_field_gen_name):
    try:
        ...
        return sum([EC_R_plot([S[i](c) for c in ainvs], xmin, xmax, ymin, ymax, cols[i], "$" +
                              base_field_gen_name +
                              r" \mapsto$ " +
                              str(S[i].im_gens()[0].n(20)) +
                              r"$\dots$") for i in range(n1)])
    except Exception:
...
class ECNF(object):
    ...
    def make_E(self):
        ...
        if self.cm and self.galois_images != '?':
            self.cm_ramp = [
                p for p in ZZ(
                    self.cm).support() if p not in self.non_surjective_primes]
        ...
        if self.cm:
            if self.signature == [
                    0, 1] and ZZ(-self.abs_disc * self.cm).is_square():
        ...
        if self.base_change:
            ...
            self.properties += [('Base change', 'yes: %s' %
                                 ','.join([str(lab) for lab in self.base_change]))]
        else:
        ...
        for E0 in self.base_change:
            self.friends += [(r'Base change of %s /\(\Q\)' %
                              E0, url_for("ec.by_ec_label", label=E0))]
...
code_names = {
    'field': 'Define the base number field',
