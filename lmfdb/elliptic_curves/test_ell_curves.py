...
class EllCurveTest(LmfdbTest):
    ...
    def test_by_iso_label(self):
        ...
        L = self.tc.get('/EllipticCurve/Q/12350/s')
        assert 'You should be redirected automatically to target URL:' in L.get_data(
            as_text=True)
    ...
    def test_Cond_search(self):
        L = self.tc.get(
            '/EllipticCurve/Q/?start=0&conductor=1200&jinv=&rank=&torsion=&torsion_structure=&sha=&optimal=&surj_primes=&surj_quantifier=include&nonsurj_primes=&count=100')
    ...
    def test_Weierstrass_search(self):
        ...
        assert 'You should be redirected automatically to target URL:' in L.get_data(
            as_text=True)
        assert '/EllipticCurve/Q/%5B1%2C2%2C3%2C4%2C5%5D/' in L.get_data(
            as_text=True)

    def test_j_search(self):
        L = self.tc.get(
            '/EllipticCurve/Q/?start=0&conductor=&jinv=2000&rank=&torsion=&torsion_structure=&sha=&optimal=&surj_primes=&surj_quantifier=include&nonsurj_primes=&count=100')
    ...
    def test_jbad_search(self):
        L = self.tc.get(
            '/EllipticCurve/Q/?start=0&conductor=&jinv=2.3&rank=&torsion=&torsion_structure=&sha=&optimal=&surj_primes=&surj_quantifier=include&nonsurj_primes=&count=100')
        assert 'Error' in L.get_data(as_text=True)
        assert 'rational number' in L.get_data(as_text=True)

    def test_tors_search(self):
        L = self.tc.get(
            '/EllipticCurve/Q/?start=0&conductor=&jinv=&rank=&torsion=&torsion_structure=[7]&sha=&optimal=&surj_primes=&surj_quantifier=include&nonsurj_primes=&count=100')
    ...
    def test_SurjPrimes_search(self):
        self.check_args_with_timeout(
            '/EllipticCurve/Q/?start=0&conductor=&jinv=&rank=&torsion=&torsion_structure=&sha=&optimal=&surj_primes=2&surj_quantifier=include&nonsurj_primes=&count=100',
            '[0, 0, 1, -270, -1708]')

    def test_NonSurjPrimes_search(self):
        self.check_args_with_timeout(
            '/EllipticCurve/Q/?start=0&conductor=&jinv=&rank=&torsion=&torsion_structure=&sha=&optimal=&surj_primes=&surj_quantifier=exactly&nonsurj_primes=37&count=100',
    ...
    def test_sha(self):
        L = self.tc.get(
            'EllipticCurve/Q/?start=0&conductor=&jinv=&rank=2&torsion=&torsion_structure=&sha=2-&optimal=&surj_primes=&surj_quantifier=include&nonsurj_primes=&count=100')
        assert '[0, 1, 0, -73824640, -244170894880]' in L.get_data(
            as_text=True)
        assert '226920.h1' in L.get_data(as_text=True)
        L = self.tc.get(
            'EllipticCurve/Q/?start=0&conductor=&jinv=&rank=&torsion=&torsion_structure=&sha=81&optimal=&surj_primes=&surj_quantifier=include&nonsurj_primes=&count=100')
        assert '101592.p1' in L.get_data(as_text=True)
        L = self.tc.get(
            'EllipticCurve/Q/?start=0&conductor=&jinv=&rank=&torsion=&torsion_structure=&sha=8.999&optimal=&surj_primes=&surj_quantifier=include&nonsurj_primes=&count=100')
        assert 'Error' in L.get_data(as_text=True)
