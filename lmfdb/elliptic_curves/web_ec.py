...
RZB_URL_PREFIX = "http://users.wfu.edu/rouseja/2adic/"
...
def gl2_subgroup_data(label):
    ...
    if data['genus'] > 0:
        info += row_wrap('Newforms',
                         ''.join(['<a href="%s">%s</a>' % (cmf_url_for_label(x),
                                                           x) for x in data['newforms']]))
        info += row_wrap('Analytic rank', data['rank'])
        if data['genus'] == 1 and data['model']:
            info += row_wrap('Model', '<a href="%s">%s</a>' %
                             (url_for('ec.by_ec_label', label=data['model']), data['model']))
    info += "</table>\n"
    return info
...
def latex_equation(ainvs):
    ...
    return ''.join(
        [
            ...
            '+y' if a3 else '',
            '=x^3',
            '+x^2' if a2 == 1 else '-x^2' if a2 == -
            1 else '',
            '{:+}x'.format(a4) if abs(a4) > 1 else '+x' if a4 == 1 else '-x' if a4 == -
            1 else '',
...
def homogeneous_latex_equation(ainvs):
    ...
    return ''.join(
        [
            ...
            '+yz^2' if a3 else '',
            '=x^3',
            '+x^2z' if a2 == 1 else '-x^2z' if a2 == -
            1 else '',
            '{:+}xz^2'.format(a4) if abs(a4) > 1 else '+xz^2' if a4 == 1 else '-xz^2' if a4 == -
            1 else '',
            '{:+}z^3'.format(a6) if abs(a6) > 1 else '+z^3' if a6 == 1 else '-z^3' if a6 == -
            1 else '',
            r'\)'])
...
def short_latex_equation(ainvs):
    ...
    B = 54 * a1**6 + 648 * a1**4 * a2 - 1944 * a1**3 * a3 + 2592 * a1**2 * a2**2 - 3888 * a1**2 * \
        a4 - 7776 * a1 * a2 * a3 + 3456 * a2**3 - 15552 * a2 * a4 + 11664 * a3**2 + 46656 * a6
    ...
    return ''.join(
        [
            r'\(y^2=x^3',
            '{:+}x'.format(A) if abs(A) > 1 else '+x' if A == 1 else '-x' if A == -
            1 else '',
            '{:+}'.format(B) if B else '',
            r'\)'])
...
def latex_equations(ainvs):
    return [
        latex_equation(ainvs),
        homogeneous_latex_equation(ainvs),
        short_latex_equation(ainvs)]
...
class WebEC():
    ...
    def make_curve(self):
        ...
        if N < OPTIMALITY_BOUND:
            ...
            if self.optimality == 1:
                ...
                if self.Cnumber == 1:
                    ...
                    opt_curve = db.ec_curvedata.lucky({'Ciso': self.Ciso, 'Cnumber': 1}, projection=[
                                                      'Clabel', 'lmfdb_label', 'optimality'])
        ...
        self.friends = [
            ('Isogeny class ' + self.class_name, self.class_url), ('Minimal quadratic twist %s %s' %
                                                                   (data['minq_info'], data['minq_label']), url_for(
                                                                       ".by_ec_label", label=data['minq_label'])), ('All twists ', url_for(
                                                                           ".rational_elliptic_curves", jinv=data['j_invariant']))]
        ...
        self.properties = [
            ('Label',
             self.Clabel if self.label_type == 'Cremona' else self.lmfdb_label),
            (None,
             self.plot_link),
            ('Conductor',
             prop_int_pretty(
            ...
            ('CM',
             ...
             data['CM']),
            ('Rank',
             'unknown' if self.mwbsd['rank'] == '?' else prop_int_pretty(
                 self.mwbsd['rank'])),
            ('Torsion structure',
             (r'\(%s\)' %
    ...
    def make_mwbsd(self):
        ...
        mwbsd['generators'] = [raw_typeset(weighted_proj_to_affine_point(
            P)) for P in mwbsd['gens']] if mwbsd['ngens'] else ''
        mwbsd['heights'] = [RR(h) for h in mwbsd['heights']]

        # Mordell-Weil group
        invs = [0 for a in range(self.rank)] + \
            [n for n in self.torsion_structure]
        ...
        if mwbsd['torsion'] == 1:
            ...
            mwbsd['tor_gens'] = raw_typeset(
                ', '.join(
                    str(P) for P in tor_gens_tmp), ', '.join(
                    web_latex(P) for P in tor_gens_tmp))
