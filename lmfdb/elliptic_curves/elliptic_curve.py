...
def rational_elliptic_curves(err_args=None):
    ...
    conductor_list = dict([(r, r) for r in ["%s-%s" %
                                            (start, end -
                                             1) for start, end in zip(conductor_list_endpoints[:-
                                                                                               1], conductor_list_endpoints[1:])]])
...
def ec_lookup_equation(input_str):
    ...
    try:
        E = EllipticCurve_from_Weierstrass_polynomial(
            S(ec_defining_poly)).minimal_model()
    except ValueError:
...
class EC_download(Downloader):
    ...
    function_body = {
        "magma": [
            # convert ai from list to sequence
...
def render_congruent_number_data():
    ...
    if 'lookup' in info:
        return redirect(
            url_for(
                ".render_single_congruent_number",
                n=info['lookup']))
...
class ECSearchArray(SearchArray):
    ...
    def __init__(self):
        ...
        # ℤ is &#8484; in html
        torsion_opts = ([("", ""), ("[]", "trivial")] + [("%s" %
                                                          n, "order %s" %
                                                          n) for n in range(4, 16, 4)] + [("[%s]" %
                                                                                           n, "ℤ/%sℤ" %
                                                                                           n) for n in range(2, 13) if n != 11] + [("[2,%s]" %
                                                                                                                                    n, "ℤ/2ℤ&oplus;ℤ/%sℤ" %
                                                                                                                                    n) for n in range(2, 10, 2)])
        torsion = SelectBox(
            name="torsion",
            label="Torsion",
                    ...
                    ('CM',
                     'potential CM')] + [('-4,-16',
                                         ...
                                         ('-3,-12,-27',
                                          'CM field Q(sqrt(-3))'),
                                         ('-7,-28',
                                          'CM field Q(sqrt(-7))')] + [('-%d' % d,
                                                                       'CM discriminant -%d' % d) for d in [3,
                                                                                                            4,
                                                                                                            7,
                                                                                                            8,
                                                                                                            11,
