...
def download_search(info):
    ...
    s = com1 + "\n"
    s += com + \
        ' Elliptic curves downloaded from the LMFDB downloaded on {}.\n'.format(mydate)
...
class ECSearchArray(SearchArray):
    ...
    def __init__(self):
        ...
        cm = SelectBox(
            name="include_cm", label="CM", knowl="ec.complex_multiplication", options=[
                ('', ''), ('only', 'potential CM'), ('exclude', 'no potential CM')])
        tor_opts = ([("", ""), ("[]", "trivial")] + [("[%s]" %
                                                      n, "C%s" %
