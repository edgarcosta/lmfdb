...
def distray(P, Q):
    ...
    if P[0] * (P[1] - theta).cos() >= R0:
        # The minimum distance to the theta line occurs on the ray, so we can
        # just use the right triangle
...
# The HSV color coordiantes are obtained from the following dictionary:
...
def pack(rdata, R0, rmax):
    ...
    if thetasum < 2 * pi:
        ...
        if all(distxy(pos[i], pos[(i + 1) %
                                  len(pos)]) >= radii[i] + radii[(i + 1) %
                                                                 len(pos)] for i in range(len(pos))):
...
def find_packing(ccdata):
    ...
    for (n, o) in ccdata:
        n, o = ZZ(n), ZZ(o)
        if o != 1:
            by_pcount[sum(e for (p, e) in o.factor())
                      ].append((get_radius(n), o))
    r0 = R = get_radius(1)
