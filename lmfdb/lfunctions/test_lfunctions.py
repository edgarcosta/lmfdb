...
class LfunctionTest(LmfdbTest):
    ...
    def test_Lec(self):
        ...
        L = self.tc.get(
            '/L/EllipticCurve/2.0.1879.1/1.0.1/a/',
            follow_redirects=True)
        #assert '/SatoTateGroup/1.2.' in L.get_data(as_text=True)
    ...
    def test_Lgl2maass(self):
        ...
        L = self.tc.get(
            '/L/Zeros/ModularForm/GL2/Q/Maass/4f5695df88aece2afe000021/')
        ...
        L = self.tc.get(
            '/L/Zeros/ModularForm/GL2/Q/Maass/4f55571b88aece241f000013/')
    ...
    def test_Ldedekind(self):
        ...
        assert 'chi_{172}' in L.get_data(
            as_text=True) and 'chi_{43}' in L.get_data(
            as_text=True)
    ...
    def test_Lartin(self):
        ...
        assert '0.1740363269' in L.get_data(as_text=True)
        L = self.tc.get(
            '/L/Zeros/ArtinRepresentation/2.23.3t2.b.a/',
            follow_redirects=True)
        assert '5.1156833288' in L.get_data(as_text=True)
        L = self.tc.get(
            '/L/ArtinRepresentation/4.1609.5t5.a.a/',
            follow_redirects=True)
        assert '0.0755586459' in L.get_data(as_text=True)
        L = self.tc.get(
            '/L/Zeros/ArtinRepresentation/4.1609.5t5.1c1/',
            follow_redirects=True)
    ...
    def test_LHGMZeros(self):
        L = self.tc.get(
            '/L/Zeros/Motive/Hypergeometric/Q/A2.2.2.2_B1.1.1.1/t-1.1/')
    ...
    def test_errorMessages(self):
        ...
        assert 'L-function of Hilbert form of non-trivial character not implemented yet' in L.get_data(
            as_text=True)
        ...
        assert 'Error constructing Artin representation' in L.get_data(
            as_text=True)
        ...
        assert 'This Elliptic curve has complex multiplication and the symmetric power of its L-function is then not primitive.' in L.get_data(
            as_text=True)
