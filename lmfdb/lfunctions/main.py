...
def learnmore_list(path=None, remove=None):
    if path:
        ...
        learnmore = [
            ('Source and acknowledgments', url_for(
                '.source', prepath=prepath)), ('Completeness of the data', url_for(
                    '.completeness', prepath=prepath)), ('Reliability of the data', url_for(
                        '.reliability', prepath=prepath)), ('L-function labels', url_for('.labels'))]
    else:
...
def process_search(res, info, query):
    ...
    for L in res:
        ...
        mus = [
            latex(
                mu_real +
                L['analytic_normalization']) if mu_imag == 0 else display_complex(
                mu_real +
        ...
        nus = [
            latex(
                nu_real_doubled *
                0.5 +
                L['analytic_normalization']) if nu_imag == 0 else display_complex(
                nu_real_doubled *
                0.5 +
                L['analytic_normalization'],
                nu_imag,
                3) for nu_real_doubled,
            nu_imag in zip(
                L['nu_real_doubled'],
                L['nu_imag'])]
...
def url_for_lfunction(label):
    try:
        kwargs = dict(zip(('degree', 'conductor', 'character',
                           'gamma_real', 'gamma_imag', 'index'), label.split('-')))
...
def by_old_degree(degree):
    return redirect(
        url_for(
            ".by_url_degree_conductor_character_spectral",
            degree=degree))
...
@l_function_page.route("/rational/<int:degree>",
                       defaults={elt: None for elt in ['conductor',
                                                       'character',
                                                       'spectral_label']})
...
@l_function_page.route("/<int:degree>/<conductor>",
                       defaults={elt: None for elt in ['character',
                                                       'spectral_label']})
...
def l_function_cuspform_browse_page():
    ...
    return render_template(
        "cuspformGL2.html",
        title=r'L-functions of cusp forms on \(\Gamma_1(N)\)',
        **info)
...
def l_function_maass_browse_page():
    ...
    return render_template(
        "MaassformGL2.html",
        title='L-functions of GL(2) Maass forms of weight 0',
        **info)
...
def l_function_maass_gln_browse_page(degree):
    ...
    if not contents:
        return abort(404)
    info = {"bread": get_bread([("Maass form", url_for(
        '.l_function_maass_gln_browse_page', degree='degree' + str(degree)))])}
    info["contents"] = contents
    info["learnmore"] = learnmore_list()
    return render_template(
        "MaassformGLn.html",
        title='L-functions of GL(%s) Maass forms' %
        degree,
        **info)
...
def l_function_ec_sym2_browse_page():
    ...
    return render_template(
        "ellipticcurve.html",
        title='Symmetric square L-functions of elliptic curves',
        **info)
...
def l_function_ec_sym3_browse_page():
    ...
    return render_template(
        "ellipticcurve.html",
        title='Symmetric cube L-functions of elliptic curves',
        **info)
...
def l_function_genus2_browse_page():
    ...
    return render_template(
        "genus2curve.html",
        title='L-functions of genus 2 curves',
        **info)
...
def l_function_browse_page(degree, gammasignature):
    ...
    if not contents:
        return abort(404)
    info = {"bread": get_bread([(gammasignature,
                                 url_for('.l_function_browse_page',
                                         degree='degree' + str(degree),
                                         gammasignature=gammasignature))])}
    info["contents"] = contents
    info["learnmore"] = learnmore_list()
    return render_template(
        "MaassformGLn.html", title='L-functions of degree %s and signature %s' %
        (degree, nice_gammasignature), **info)
...
def l_function_dirichlet_page(modulus, number):
    ...
    # database
    return abort(
        404, "L-function for dirichlet character with label %s.%s not found" %
        (modulus, number))
...
def l_function_ec_page(conductor_label, isogeny_class_label):
    ...
    return abort(
        404,
        "L-function for elliptic curve isogeny class with label %s not found" %
        label)
...
def l_function_cmf_orbit(level, weight, char_orbit_label, hecke_orbit):
    ...
    return abort(
        404,
        "L-function for classical modular form with label %s not found" %
        label)
...
def l_function_bmf_page(field, level, suffix):
    ...
    return abort(
        404,
        "L-function for Bianchi modular form with label %s not found" %
        label)
...
def l_function_genus2_page(cond, x):
    ...
    return abort(
        404,
        "L-function for genus 2 curve with label %s not found" %
        label)
        ...
        info['friends'] = [
            (r'\(\mathbb Q\)',
             url_for(
                 ...
                 label='1.1.1.1')),
            (r'Dirichlet character \(\chi_{1}(1,\cdot)\)',
             url_for(
                 'characters.render_Dirichletwebpage',
                 modulus=1,
                 number=1)),
            ('Artin representation 1.1.1t1.1c1',
             url_for(
        ...
        if L.m == 2:
            info['bread'] = get_bread(
                [
                    ...
                    (L.origin_label,
                     url_for(
                         '.l_function_ec_sym_page_label',
                         label=L.origin_label,
                         power=L.m))])
        elif L.m == 3:
            info['bread'] = get_bread(
                [
...
def set_zeroslink_and_plotlink(L, args):
    ...
    if hasattr(
        ...
        hasattr(
            L,
            'sageLfunction') and L.sageLfunction):
...
def set_navi(L):
    ...
    if L.Ltype() == 'maass' and L.group == 'GL2':
        ...
        if mod > 1:
            ...
            prev_data = (
                "previous", Lpattern %
                (pmod, pnum) if pmod > 1 else r"\(\zeta(s)\)", url_for(
                    '.l_function_dirichlet_page', modulus=pmod, number=pnum))
        else:
            prev_data = ('', '', '')
        nmod, nnum = WebDirichlet.nextprimchar(mod, num)
        next_data = (
            "next", Lpattern %
            (nmod, nnum) if nmod > 1 else r"\(\zeta(s)\)", url_for(
                '.l_function_dirichlet_page', modulus=nmod, number=nnum))
...
def generateLfunctionFromUrl(*args, **kwds):
    ...
    if args[0] == 'ModularForm' and args[1] == 'GL2' and args[2] == 'TotallyReal' and args[4] == 'holomorphic':  # Hilbert modular form
        ...
        return SymmetricPowerLfunction(
            power=args[1],
            underlying_type=args[2],
            field=args[3],
            conductor=args[4],
            isogeny=args[5])
...
def download_route_wrapper(f):
    ...
    def wrapper(*args, **kwds):
        ...
        if not db.lfunc_lfunctions.exists({'label': label}):
            return render_lfunction_exception(
                "There is no L-function in the database with label '%s'" %
                label)
...
def processGenus2CurveNavigation(startCond, endCond):
    ...
    for cond, x in iso_list:
        ...
        counter += 1
        s += '<td><a href="' + \
            url_for('.l_function_genus2_page', cond=cond, x=x) + '">%s</a></td>\n' % label
...
def processSymPowerEllipticCurveNavigation(startCond, endCond, power):
    ...
    for cond, iso in iso_list:
        ...
        counter += 1
        s += '<td><a href="' + url_for('.l_function_ec_sym_page', power=str(
            power), conductor=cond, isogeny=iso) + '">%s</a></td>\n' % label
