...
def paintSvgFileAll(glslist):  # list of group and level
    ...
    for gls in glslist:
        ...
        for l in db.lfunc_lfunctions.search(
                {'group': group, 'conductor': level}, ['origin', 'root_number']):
...
def getOneGraphHtmlHolo(condmax):
    ...
    logger.debug(pic[0])
    ans = ("<embed  src='%s' width='%s' height='%s' type='image/svg+xml' " %
           pic + "pluginspage='http://www.adobe.com/svg/viewer/install/'/>\n")
    ans += "<br/>\n"
...
def paintSvgHoloNew(condmax):
    ...
    for nf in db.mf_newforms.search(
        {
            'analytic_conductor': {
            '$lte': condmax}},
            projection=[
...
def paintSvgHolo(Nmin, Nmax, kmin, kmax):
                ...
                for MF in newsp[1]:
                    ...
                    if numberwithlabel > maxdots:  # if more than maxdots in orbit, use number as symbol
                        ...
                        if (self_dual > 0 and numpluslabels > 1) or (
                            ...
                            ybase += ydotspacing
                        ans += "<a xlink:href='" + \
                            url_for('not_yet_implemented') + "' target='_top'>\n"
    ...
    #  TODO: Implement when there is more than maxdots forms
                        ...
                        for number in range(numberwithlabel):
                            ...
                            ans += "<title>" + \
                                str((x, y)).replace("u", "").replace("'", "") + "</title>"
                            ans += "</circle></a>\n"
                    ...
                    if signfe == signtmp:  # we find an orbit with sign of "signtmp"
                        ...
                        for n in range(numberwithlabel):
                            orbitdescriptionlist.append(
                                {'label': label, 'number': n, 'color': signcolour})
    ...
    for orbit in urlinfo['space']['orbits']:
        ...
        if len(orbit) > 1 and len(orbit) <= maxdots:
            ans += myline(offset,
                          ...
                          lincomb(1,
                                  orbitbase,
                                  (len(orbit) - 1) * dotspacing[1],
                                  edge[1]),
                          dimensioninfo['connectinglinewidth'],
                          "",
                          appearanceinfo['edgecolor'])
            ans += "\n"
            ...
            ans += mytext(len(orbit),
                          offset,
                          scale,
                          orbitbase,
                          "",
                          appearanceinfo['fontsize'],
                          appearanceinfo['fontweight'],
                          orbitcolor)
            ans += "</a>"
            ans += "\n"
