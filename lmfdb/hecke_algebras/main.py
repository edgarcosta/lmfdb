...
def download_search(info):
    ...
    for c, rr in enumerate(res):
        ...
        if 'ell' in info["query"]:
            ...
            s += ",".join([entry(r) for r in [sage_eval(rr['hecke_op'])[i]
...
def render_hecke_algebras_webpage_l_adic(**args):
    ...
    return render_template(
        "hecke_algebras_l_adic-single.html",
        info=info,
        credit=credit,
        title=t,
        bread=bread,
...
def history_page():
    ...
    return render_template(
        "single.html",
        kid='hecke_algebras.history',
        credit=credit,
        title=t,
        bread=bread,
        learnmore=learnmore_list_remove('History'))
...
def download_hecke_algebras_full_lists_op(**args):
    ...
    outstr += ",\\\n".join([entry(r) for r in [sage_eval(res['hecke_op'])[i]
                                               for i in range(0, res['num_hecke_op'])]])
    outstr += ']'
