...
class WebCharGroup(WebCharObject):
    ...
    def first_chars(self):
        ...
        for i, c in enumerate(
            Integers(
                self.modulus).list_of_elements_of_multiplicative_group()):
            r.append(c)
            if i > self.maxrows:
        ...
        return {
            'sage': [
                ...
                'H = DirichletGroup({}, base_ring=CyclotomicField({}))'.format(
                    self.modulus,
                    self.chi.sage_zeta_order(
                        self.order)),
                'M = H._module',
                'chi = DirichletCharacter(H, M([{}]))'.format(
                    ','.join(
                        str(val) for val in self._genvalues_for_code)),
            ],
            'pari': '[g,chi] = znchar(Mod(%i,%i))' % (self.number,
                                                      self.modulus),
        }
...
class WebDBDirichletOrbit(WebChar, WebDBDirichlet):
    ...
    def __init__(self, **kwargs):
        ...
        self.orbit_data = self.get_orbit_data(
            self.orbit_label)  # this is the meat
        self.maxrows = 30
        self.rowtruncate = False
    ...
    def _set_galoisorbit(self, orbit_data):
        ...
        self.galoisorbit = list(
            self._char_desc(
                num,
                prim=orbit_data['is_primitive']) for num in self.galorbnums)
