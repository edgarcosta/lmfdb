...
def render_hmf_webpage(**args):
    ...
    if Lfun:
        ...
        # old code
        if hmf_field['narrow_class_no'] == 1 and nf.disc()**2 * \
                data['level_norm'] < 40000:
            info['friends'] = [
                ('L-function',
                 url_for(
    ...
    if not AL_eigs:  # empty list
        ...
        info['AL_eigs'] = [{'eigenvalue': teXify_pol(al[1]), 'prime_ideal': teXify_pol(
            al[0]), 'prime_norm': al[0][1:al[0].index(',')]} for al in data['AL_eigenvalues']]
...
class HMFSearchArray(SearchArray):
    ...
    sorts = [
        ("", "base field", [
            'deg', 'disc', 'level_norm', 'level_label', 'label_nsuffix']), ("level_norm", "level norm", [
                'level_norm', 'deg', 'disc', 'level_label', 'label_nsuffix']), ("dimension", "dimension", [
                    'dimension', 'deg', 'disc', 'level_norm', 'level_label', 'label_nsuffix'])]
